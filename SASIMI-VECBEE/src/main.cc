...
parser Cmdline_Parser(int argc, char *argv[]) {
  parser option;
  option.add<string>("input", 'i', "original circuit file", false,
                     "data/su/c880.blif");
  ...
  option.add<int>("maxLevel", '\0', "max TFO cut level", false, INT_MAX,
                  range(1, INT_MAX));
  option.add<int>("maxIter", '\0', "max number of iterations", false, INT_MAX,
                  range(1, INT_MAX));
...
int main(int argc, char *argv[]) {
  ...
  int maxIterations = option.get<int>("maxIter");
  ...
  SASIMI_Manager_t sasimiMng(pNtk, frameNumber, maxLevel, metricType,
                             errorBound, maxIterations);
