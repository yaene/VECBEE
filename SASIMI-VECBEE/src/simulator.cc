...
Simulator_t::Simulator_t(Abc_Ntk_t *pNtk, int nFrame) {
  DASSERT(Abc_NtkIsAigLogic(pNtk) || Abc_NtkIsSopLogic(pNtk) ||
              Abc_NtkIsMappedLogic(pNtk),
          "network must be in aig, sop or mapped logic");
  this->pNtk = pNtk;
  this->nFrame = nFrame;
  this->nBlock = (nFrame % 64) ? ((nFrame >> 6) + 1)
                               : (nFrame >> 6); // 64 frames per block
  this->nLastBlock =
      (nFrame % 64) ? (nFrame % 64) : 64; // number of frames in last block
...
void Simulator_t::Input(unsigned seed) {
  ...
  variate_generator<random::mt19937, random::uniform_int_distribution<uint64_t>>
      randomPI(engine, unf);
...
void Simulator_t::Simulate() {
  ...
  int i = 0;
  if (Abc_NtkIsAigLogic(pNtk)) {
    Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pObj, i) UpdateAigNode(pObj);
  } else if (Abc_NtkIsSopLogic(pNtk)) {
    Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pObj, i) UpdateSopNode(pObj);
  } else if (Abc_NtkIsMappedLogic(pNtk)) {
    Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pObj, i) UpdateMapNode(pObj);
  } else {
    DASSERT(0);
  }
  ...
  Abc_NtkForEachPo(pNtk, pObj, i) {
    ...
    values[pObj->Id].assign(values[pFanin->Id].begin(),
                            values[pFanin->Id].end());
  }
}
    ...
    if (!Abc_NodeIsConst(pObj)) {
      ...
      for (auto &pCutNode : djCuts[pObj->Id]) {
        ...
        for (int k = 0; k < nBlock; ++k)
          bdCuts[pObj->Id][j][k] =
              values[pCutNode->Id][k] ^ tmpValues[pCutNode->Id][k];
        ++j;
      }
    }
  }
...
void Simulator_t::SimulateOneCutNtks() {
  ...
  Abc_NtkForEachNode(pNtk, pObj, i) {
    if (!Abc_NodeIsConst(pObj)) {
      ...
      for (int j = 0; j < nBlock; ++j) {
        tmpValues[pObj->Id][j] = ~values[pObj->Id][j];
        // cout << "filp " << Abc_ObjName(pObj) << "," << values[pObj->Id][j] <<
        // "," << tmpValues[pObj->Id][j] << endl;
      }
      // simulate
      Abc_NtkIncrementTravId(pNtk);
      Abc_NodeSetTravIdCurrent(pObj);
      ...
      if (Abc_NtkIsAigLogic(pNtk)) {
        ...
        for (auto &pInner : cutNtks[pObj->Id]) {
          UpdateMapObjCutNtk(pInner);
          // cout << Abc_ObjName(pInner) << " old value " <<
          // values[pInner->Id][0] << " new value " << tmpValues[pInner->Id][0]
          // << endl;
        }
      } else
        DASSERT(0);
      ...
      for (int j = 0; j < nPo; ++j) {
        if (Ckt_GetBit(sinks[pObj->Id][j >> 6],
                       static_cast<uint64_t>(j) & static_cast<uint64_t>(63))) {
          Abc_Obj_t *pCutNode = oneCuts[pObj->Id][j];
          if (pCutNode != nullptr) {
            bdOneCuts[pObj->Id][j].resize(nBlock);
            for (int k = 0; k < nBlock; ++k) {
              bdOneCuts[pObj->Id][j][k] =
                  values[pCutNode->Id][k] ^ tmpValues[pCutNode->Id][k];
              // cout << Abc_ObjName(pObj) << "," << j << "," << k << "," <<
              // bdOneCuts[pObj->Id][j][k] << endl;
            }
          }
        }
      }
...
void Simulator_t::SimulateResub(Abc_Obj_t *pOldObj, void *pResubFunc,
  ...
  if (Abc_NtkIsAigLogic(pNtk)) {
    Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pObj, i) {
      if (pObj != pOldObj)
        ...
        UpdateAigNodeResub(pObj, static_cast<Hop_Obj_t *>(pResubFunc),
                           vResubFanins);
    }
  } else if (Abc_NtkIsSopLogic(pNtk)) {
    Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pObj, i) {
  ...
  Abc_NtkForEachPo(pNtk, pObj, i) {
    Abc_Obj_t *pFanin = Abc_ObjFanin0(pObj);
    tmpValues[pObj->Id].assign(tmpValues[pFanin->Id].begin(),
                               tmpValues[pFanin->Id].end());
  }
  Vec_PtrFree(vNodes);
}
...
void Simulator_t::SimulateSASIMI(Abc_Obj_t *pTS, Abc_Obj_t *pSS, bool isInv) {
  ...
  int i = 0;
  if (Abc_NtkIsAigLogic(pNtk)) {
    Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pObj, i) if (pObj != pTS)
        UpdateAigNode(pObj, true);
  } else if (Abc_NtkIsSopLogic(pNtk)) {
    DASSERT(0);
  } else if (Abc_NtkIsMappedLogic(pNtk)) {
    Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pObj, i) {
      ...
      // cout << Abc_ObjName(pObj) << "," << (tmpValues[pObj->Id][0] &
      // (uint64_t)(1)) << endl;
    }
  } else {
    DASSERT(0);
...
void Simulator_t::UpdateAigNode(Abc_Obj_t *pObj, bool isTemp) {
  auto &val = isTemp ? tmpValues : values;
  ...
  Vec_PtrForEachEntry(Hop_Obj_t *, vHopNodes, pHopObj, i) maxHopId =
      max(maxHopId, pHopObj->Id);
  Hop_ManForEachPi(pMan, pHopObj, i) maxHopId = max(maxHopId, pHopObj->Id);
  vector<tVec> interValues(maxHopId + 1);
  Vec_PtrForEachEntry(Hop_Obj_t *, vHopNodes, pHopObj, i)
      interValues[pHopObj->Id]
          .resize(nBlock);
  unordered_map<int, tVec *> hop2Val;
  Abc_Obj_t *pFanin = nullptr;
  Abc_ObjForEachFanin(pObj, pFanin, i) hop2Val[Hop_ManPi(pMan, i)->Id] =
      &val[pFanin->Id];

  // special case for inverter or buffer
  if (pRootR->Type == AIG_PI) {
    pFanin = Abc_ObjFanin0(pObj);
    val[pObj->Id].assign(val[pFanin->Id].begin(), val[pFanin->Id].end());
  }
  ...
  Vec_PtrForEachEntry(Hop_Obj_t *, vHopNodes, pHopObj, i) {
    ...
    tVec &val0 = Hop_ObjIsPi(pHopFanin0) ? *hop2Val[pHopFanin0->Id]
                                         : interValues[pHopFanin0->Id];
    tVec &val1 = Hop_ObjIsPi(pHopFanin1) ? *hop2Val[pHopFanin1->Id]
                                         : interValues[pHopFanin1->Id];
    tVec &out = (pHopObj == pRootR) ? val[pObj->Id] : interValues[pHopObj->Id];
  ...
  if (Hop_IsComplement(pRoot)) {
    for (int j = 0; j < nBlock; ++j)
      val[pObj->Id][j] ^= static_cast<uint64_t>(ULLONG_MAX);
  }
...
void Simulator_t::UpdateMapNode(Abc_Obj_t *pObj, bool isTmpValue) {
  ...
  // update sop
  char *pSop =
      static_cast<char *>((static_cast<Mio_Gate_t *>(pObj->pData))->pSop);
  int nVars = Abc_SopGetVarNum(pSop);
...
void Simulator_t::UpdateAigObjCutNtk(Abc_Obj_t *pObj) {
  ...
  if (Abc_ObjIsPo(pObj)) {
    ...
    if (Abc_NodeIsTravIdCurrent(pDriver))
      tmpValues[pObj->Id].assign(tmpValues[pDriver->Id].begin(),
                                 tmpValues[pDriver->Id].end());
    else
      tmpValues[pObj->Id].assign(values[pDriver->Id].begin(),
                                 values[pDriver->Id].end());
    return;
  }
  ...
  if (pRootR->Type == AIG_PI) {
    ...
    if (Abc_NodeIsTravIdCurrent(pFanin))
      ...
      tmpValues[pObj->Id].assign(values[pFanin->Id].begin(),
                                 values[pFanin->Id].end());
  }
  ...
  Vec_PtrForEachEntry(Hop_Obj_t *, vHopNodes, pHopObj, i) {
    ...
    tVec &val1 = Hop_ObjIsPi(pHopFanin1) ? *hop2Val[pHopFanin1->Id]
                                         : interValues[pHopFanin1->Id];
    tVec &out =
        (pHopObj == pRootR) ? tmpValues[pObj->Id] : interValues[pHopObj->Id];
    bool isFanin0C = Hop_ObjFaninC0(pHopObj);
    bool isFanin1C = Hop_ObjFaninC1(pHopObj);
    if (!isFanin0C && !isFanin1C) {
      for (int j = 0; j < nBlock; ++j)
...
void Simulator_t::UpdateSopObjCutNtk(Abc_Obj_t *pObj) {
  ...
  for (char *pCube = pSop; *pCube; pCube += nVars + 3) {
    ...
    for (int i = 0; pCube[i] != ' '; i++) {
      Abc_Obj_t *pFanin = Abc_ObjFanin(pObj, i);
      tVec &valueFi = Abc_NodeIsTravIdCurrent(pFanin) ? tmpValues[pFanin->Id]
                                                      : values[pFanin->Id];
      switch (pCube[i]) {
      case '-':
        continue;
...
void Simulator_t::UpdateAigNodeResub(Abc_Obj_t *pObj, Hop_Obj_t *pResubFunc,
                                     Vec_Ptr_t *vResubFanins) {
  Hop_Man_t *pMan = static_cast<Hop_Man_t *>(pNtk->pManFunc);
  Hop_Obj_t *pRoot = (pResubFunc == nullptr)
                         ? static_cast<Hop_Obj_t *>(pObj->pData)
                         : pResubFunc;
  Hop_Obj_t *pRootR = Hop_Regular(pRoot);
...
void Simulator_t::UpdateAigNodeResub(IN Abc_Obj_t *pObj,
                                     IN Hop_Obj_t *pResubFunc,
                                     IN Vec_Ptr_t *vResubFanins,
                                     OUT tVec &outValue) {
...
void Simulator_t::UpdateSopNodeResub(Abc_Obj_t *pObj, char *pResubFunc,
                                     Vec_Ptr_t *vResubFanins) {
  DASSERT(Abc_ObjIsNode(pObj));
  DASSERT(pObj->pNtk == pNtk);
  // update constant node
  ...
  // simulate
  char *pSop =
      (pResubFunc != nullptr) ? pResubFunc : static_cast<char *>(pObj->pData);
  int nVars = Abc_SopGetVarNum(pSop);
  vector<uint64_t> product(nBlock);
  if (pResubFunc != nullptr)
  ...
  for (char *pCube = pSop; *pCube; pCube += nVars + 3) {
    ...
    for (int i = 0; pCube[i] != ' '; i++) {
      Abc_Obj_t *pFanin =
          (pResubFunc != nullptr)
              ? static_cast<Abc_Obj_t *>(Vec_PtrEntry(vResubFanins, i))
              : Abc_ObjFanin(pObj, i);
      switch (pCube[i]) {
      case '-':
        continue;
...
multiprecision::int256_t Simulator_t::GetInput(int lsb, int msb,
                                               int frameId) const {
...
multiprecision::int256_t Simulator_t::GetOutput(int lsb, int msb, int frameId,
                                                bool isTmpValue) const {
...
void Simulator_t::PrintInputStream(int frameId, bool isReverse) const {
  ...
  int bitId = frameId % 64;
  if (!isReverse) {
    Abc_NtkForEachPi(pNtk, pObj, i) cout << GetValue(pObj, blockId, bitId);
  } else {
...
void Simulator_t::PrintOutputStream(int frameId, bool isReverse) const {
  ...
  int bitId = frameId % 64;
  if (!isReverse) {
    Abc_NtkForEachPo(pNtk, pObj, i) cout << GetValue(pObj, blockId, bitId);
  } else {
...
void Simulator_t::BuildCutNtks() {
  ...
  Abc_NtkForEachObj(pNtk, pObj, i) sinks[pObj->Id].resize(sinkLen, 0);
  topoIds.resize(maxId + 1);
  Vec_Ptr_t *vNodes = Abc_NtkDfs(pNtk, 0);
  Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pObj, i) topoIds[pObj->Id] = i;
  int topoId = -1;
  Abc_NtkForEachPi(pNtk, pObj, i) topoIds[pObj->Id] = topoId--;
  topoId = Vec_PtrSize(vNodes);
  Abc_NtkForEachPo(pNtk, pObj, i) topoIds[pObj->Id] = topoId++;

  // update sink POs
  Abc_NtkForEachPo(pNtk, pObj, i) Ckt_SetBit(sinks[pObj->Id][i >> 6], i);
...
void Simulator_t::UpdateFoConeAndLevel(Abc_Obj_t *pPivot) {
  ...
  Abc_Obj_t *pObj = nullptr;
  int i = 0;
  Abc_ObjForEachFanout(pPivot, pObj, i) Abc_NtkDfsReverse_rec(pObj, vNodes);

  // update fanout cone
  isInFoCone.resize((maxId >> 6) + 1);
  fill(isInFoCone.begin(), isInFoCone.end(), 0);
  Vec_PtrForEachEntryReverse(Abc_Obj_t *, vNodes, pObj, i)
      Ckt_SetBit(isInFoCone[pObj->Id >> 6],
                 static_cast<uint64_t>(pObj->Id) & static_cast<uint64_t>(63));
  Ckt_SetBit(isInFoCone[pPivot->Id >> 6],
             static_cast<uint64_t>(pPivot->Id) & static_cast<uint64_t>(63));
  ...
  // cout << "TFO cone: ";
  Vec_PtrForEachEntryReverse(Abc_Obj_t *, vNodes, pObj, i) {
    Abc_ObjForEachFanin(pObj, pFanin, j) level[pObj->Id] =
        max(level[pObj->Id], level[pFanin->Id] + 1);
...
void Simulator_t::FindOneCut(Abc_Obj_t *pPivot, int poId,
                             set<Abc_Obj_t *> &cutNtkNodes, int maxLevel) {
  DASSERT(pPivot->pNtk == pNtk);
  ...
  // static int levelGap;
  // static int nRec;
  // cout << "one cut for pivot node " << Abc_ObjName(pPivot) << " on PO " <<
  // Abc_ObjName(Abc_NtkPo(pPivot->pNtk, poId)) << ":"; if
  // (oneCuts[pPivot->Id][poId] == nullptr)
  //     // cout << "not found" << endl;
  //     assert(0);
  // else {
  ...
  //     if (gap > 1)
  //         ++nRec;
  //     cout << cnt << "," << levelGap << "," << nRec << "," << levelGap /
  //     static_cast <double> (cnt) << "," << nRec / static_cast <double> (cnt)
  //     << endl;
  // }
}

void Simulator_t::FindOneCut_rec(Abc_Obj_t *pNode, Abc_Obj_t *pPivot, int poId,
                                 set<Abc_Obj_t *> &cutNtkNodes, int maxLevel) {
  assert(!Abc_ObjIsNet(pNode));
  // if this node is already visited, skip
  if (Abc_NodeIsTravIdCurrent(pNode))
    return;
  if (!Ckt_GetBit(isInFoCone[pNode->Id >> 6],
                  static_cast<uint64_t>(pNode->Id) & static_cast<uint64_t>(63)))
    return;
  ...
  if (Abc_ObjIsCi(pNode) ||
      (Abc_NtkIsStrash(pNode->pNtk) && Abc_AigNodeIsConst(pNode)))
    return;
  ...
  if (oneCuts[pPivot->Id][poId] == nullptr &&
      level[pNode->Id] - level[pPivot->Id] <= maxLevel - 1) {
    // if (string(Abc_ObjName(pPivot)) == "[106336]")
    //     cout << "flow from node " << Abc_ObjName(pNode) << endl;
    Abc_Obj_t *pFanout = nullptr;
    int foCnt = 0;
    Abc_ObjForEachFanout(pNode, pFanout, i) {
      if (Ckt_GetBit(sinks[pFanout->Id][poId >> 6],
                     static_cast<uint64_t>(poId) & static_cast<uint64_t>(63))) {
        ++foCnt;
      }
    }
    // if (string(Abc_ObjName(pPivot)) == "[106336]")
    ...
    Abc_ObjForEachFanout(pNode, pFanout, i) {
      ...
      // if (string(Abc_ObjName(pPivot)) == "[106336]")
      //     cout << "fanout " << Abc_ObjName(pFanout) << "," <<
      //     level[pFanout->Id] << "," << level[pPivot->Id] << "," << maxLevel
      //     << endl;
      ...
      if (Ckt_GetBit(sinks[pFanout->Id][poId >> 6],
        ...
        if (abs(flow[pFanout->Id] - 1.0) <= 1e-10) {
          ...
          // if (string(Abc_ObjName(pPivot)) == "[106336]")
          //     cout << "find one cut " << Abc_ObjName(pFanout) << "," <<
          //     maxLevel << "," << endl;
        }
        // if (string(Abc_ObjName(pPivot)) == "[106336]")
        //     cout << "flow to " << Abc_ObjName(pFanout) << "," <<
        //     flow[pFanout->Id] << endl;
        if (pFanout != pPivot) {
          // if (string(Abc_ObjName(pPivot)) == "[106336]")
          //     cout << "add " << Abc_ObjName(pFanout) << " level = " <<
          //     level[pFanout->Id] - level[pPivot->Id] << " " <<
          //     Abc_ObjIsPo(pFanout) << endl;
...
void Simulator_t::SortCutNtkNodes(Abc_Obj_t *pPivot,
                                  std::set<Abc_Obj_t *> &cutNtkNodes) {
  // sort by topological order
  cutNtks[pPivot->Id].assign(cutNtkNodes.begin(), cutNtkNodes.end());
  sort(cutNtks[pPivot->Id].begin(), cutNtks[pPivot->Id].end(),
       [this](const Abc_Obj_t *pObj1, const Abc_Obj_t *pObj2) {
...
void Simulator_t::FindTFOCut(Abc_Obj_t *pPivot, int maxLevel,
                             set<Abc_Obj_t *> &cutNtkNodes) {
  ...
  if (isSkip) {
    // cout << "skip find TFO cut for pivot node " << Abc_ObjName(pPivot) <<
    // endl;
    return;
  }
...
void Simulator_t::UpdateBoolDiff(IN Abc_Obj_t *pPo, IN Vec_Ptr_t *vNodes,
                                 INOUT vector<tVec> &bdPo) {
...
void Simulator_t::UpdateBoolDiffOneCut(IN int poId, IN Vec_Ptr_t *vNodes,
                                       INOUT vector<tVec> &bdPo) {
  Abc_Obj_t *pPo = Abc_NtkPo(pNtk, poId);
  Abc_Obj_t *pObj = nullptr;
  int i = 0;
  Abc_NtkForEachPo(pNtk, pObj, i) {
    if (pObj == pPo)
      fill(bdPo[pObj->Id].begin(), bdPo[pObj->Id].end(),
           static_cast<uint64_t>(ULLONG_MAX));
    else
  ...
  Vec_PtrForEachEntryReverse(Abc_Obj_t *, vNodes, pObj, i) {
    if (Abc_NodeIsConst(pObj))
      continue;
    if (Ckt_GetBit(sinks[pObj->Id][poId >> 6],
                   static_cast<uint64_t>(poId) & static_cast<uint64_t>(63))) {
      Abc_Obj_t *pOneCut = oneCuts[pObj->Id][poId];
      if (pOneCut != nullptr) {
        for (int j = 0; j < nBlock; ++j) {
          bdPo[pObj->Id][j] =
              bdOneCuts[pObj->Id][poId][j] & bdPo[pOneCut->Id][j];
          // cout << Abc_ObjName(pObj) << "," << j << "," << bdPo[pObj->Id][j]
          // << endl;
        }
      } else {
        fill(bdPo[pObj->Id].begin(), bdPo[pObj->Id].end(), 0);
        ...
        // cout << Abc_ObjName(pObj) << "," << poId << "," << "update with
        // tfocut" << endl;
      }
    } else {
      fill(bdPo[pObj->Id].begin(), bdPo[pObj->Id].end(), 0);
...
double MeasureMRED(Abc_Ntk_t *pNtk1, Abc_Ntk_t *pNtk2, int nFrame,
  ...
  for (int k = 0; k < nFrame; ++k) {
    ...
    if (acc != 0.0)
      sum +=
          abs(1 - static_cast<bigFlt>(smlt2.GetOutput(0, nPo - 1, k, 0)) / acc);
    else
...
double MeasureResubNMED(Simulator_t *pSmlt1, Simulator_t *pSmlt2,
                        ...
                        Vec_Ptr_t *vResubFanins, bool isCheck) {
  if (isCheck)
...
double GetNMED(Simulator_t *pSmlt1, Simulator_t *pSmlt2, bool isCheck,
               bool isTmpValue) {
  if (isCheck)
    DASSERT(SmltChecker(pSmlt1, pSmlt2));
  typedef multiprecision::cpp_dec_float_100 bigFlt;
  ...
  if (isTmpValue) {
    for (int k = 0; k < nFrame; ++k)
      sum += (abs(pSmlt1->GetOutput(0, nPo - 1, k, 0) -
                  pSmlt2->GetOutput(0, nPo - 1, k, 1)));
  } else {
    for (int k = 0; k < nFrame; ++k)
      sum += (abs(pSmlt1->GetOutput(0, nPo - 1, k, 0) -
                  pSmlt2->GetOutput(0, nPo - 1, k, 0)));
  }
  // bigInt frac = (static_cast <bigInt> (nFrame)) << nPo;
  bigInt frac = (static_cast<bigInt>(1) << nPo) - 1;
  return static_cast<double>(static_cast<bigFlt>(sum) /
                             static_cast<bigFlt>(frac));
}
...
void GetOffset(IN Simulator_t *pOriSmlt, IN Simulator_t *pAppSmlt,
               IN bool isCheck, INOUT vector<vector<int8_t>> &offsets) {
  if (isCheck)
...
double MeasureResubER(Simulator_t *pSmlt1, Simulator_t *pSmlt2,
  ...
  return GetER(pSmlt1, pSmlt2, false, true) /
         static_cast<double>(pSmlt1->GetFrameNum());
}

double MeasureSASIMIER(Simulator_t *pSmlt1, Simulator_t *pSmlt2, Abc_Obj_t *pTS,
                       Abc_Obj_t *pSS, bool isInv, bool isCheck) {
  if (isCheck)
    DASSERT(SmltChecker(pSmlt1, pSmlt2));
...
int GetER(Simulator_t *pSmlt1, Simulator_t *pSmlt2, bool isCheck,
          bool isTmpValue) {
  if (isCheck)
    DASSERT(SmltChecker(pSmlt1, pSmlt2));
  ...
  vector<tVec> *pValues2 =
      isTmpValue ? pSmlt2->GetPTmpValues() : pSmlt2->GetPValues();
...
bool IOChecker(Abc_Ntk_t *pNtk1, Abc_Ntk_t *pNtk2) {
  ...
  for (int i = 0; i < nPo; ++i) {
    if (strcmp(Abc_ObjName(Abc_NtkPo(pNtk1, i)),
               Abc_ObjName(Abc_NtkPo(pNtk2, i))) != 0) {
  ...
  for (int i = 0; i < nPi; ++i) {
    if (strcmp(Abc_ObjName(Abc_NtkPi(pNtk1, i)),
               Abc_ObjName(Abc_NtkPi(pNtk2, i))) != 0) {
